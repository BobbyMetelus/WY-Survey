exports.handler = async function(event, context) {
  ...
  try {
    ...
    // Helper to safely get a number value — returns undefined if zero or missing
    const num = (key) => {
      const v = parseInt(raw[key]);
      return (!isNaN(v) && v > 0) ? v : undefined;
    };

    // Helper to safely get a text value — returns undefined if empty
    const txt = (key) => {
      const v = raw[key];
      return (v && String(v).trim() !== '') ? String(v).trim() : undefined;
    };

    // Build fields object — only include fields with real values
    const fields = {};
    ...
    // Time sliders — only send if value is greater than 0
    ...
    // Support ratings — only send if greater than 0
    ...
    console.log('Sending fields:', Object.keys(fields).length, 'fields');
